...
class Redfish::Tasks::Glassfish::TestManagedThreadFactory < Redfish::Tasks::Glassfish::BaseTaskTest
  def test_interpret_create
    ...
    executor.expects(:exec).with(equals(context),
                                 equals('create-managed-thread-factory'),
                                 equals(['--enabled', 'true', '--contextinfoenabled', 'true', '--contextinfo', 'Classloader,JNDI,Security', '--threadpriority', '6', '--property', 'SomeKey=SomeValue', '--description', 'Blah blah', 'MyThreadFactory']),
                                 equals({})).
      returns('')
  ...
  def test_create_element_where_cache_not_present_and_element_not_present
    ...
    executor.expects(:exec).with(equals(t.context), equals('list-managed-thread-factorys'), equals(%w()), equals(:terse => true, :echo => false)).
      returns('')
  ...
  def test_delete_element_where_cache_not_present_and_element_not_present
    ...
    t.options = {'name' => 'MyThreadFactory'}

    executor.expects(:exec).with(equals(t.context),
                                 equals('list-managed-thread-factorys'),
                                 equals([]),
                                 equals({:terse => true, :echo => false})).
      returns('')
  ...
  def test_delete_element_where_cache_not_present_and_element_present
    ...
    executor.expects(:exec).with(equals(t.context),
                                 equals('delete-managed-thread-factory'),
                                 equals(['MyThreadFactory']),
                                 equals({})).
      returns('')
  ...
  def test_interpret_create_and_delete
    data = {'managed_thread_factories' => resource_parameters_as_tree(:managed => true)}
  ...
  def property_prefix
    "#{raw_property_prefix}MyThreadFactory."
  end
