  ...
  module Tasks
    module Glassfish
      class ManagedExecutorService < BaseResourceTask
        PROPERTY_PREFIX = 'resources.managed-executor-service.'
        ...
        def properties_to_set_in_create
          ...
          property_map['task-queue-capacity'] = self.task_queue_capacity
        ...
        def do_create
          raise "Unable to create managed-executor-service where object_type is not 'user'" unless self.object_type == 'user'
          args = []
        ...
        def do_destroy
          context.exec('delete-managed-executor-service', [self.name])
        end

        def present?
          (context.exec('list-managed-executor-services', [], :terse => true, :echo => false) =~ /^#{Regexp.escape(self.name)}$/)
        end
      end
    end
  end
